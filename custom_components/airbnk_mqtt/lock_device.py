...
class AirbnkLockMqttDevice:
    ...
    def __init__(self, hass: HomeAssistant, device_config, mac_address = None):
        self.hass = hass
        self._callbacks = set()
        self._lockConfig = device_config
        self._lockData = self.decryptKeys(device_config["newSninfo"], device_config["appKey"])
    ...
    async def mqtt_subscribe(self):
        @callback
        async def message_received(_p0: ReceiveMessage) -> None:
            self.parse_MQTT_message(_p0.payload)

        await mqtt.async_subscribe(self.hass, BLEStateTopic % self._lockConfig[CONF_MQTT_TOPIC], msg_callback=message_received)
    ...
    def parse_MQTT_message(self, msg):
        ...
        if "DetailsBLE" in payload and ("p" and "mac" in payload["DetailsBLE"]):
            ...
            if mac_address is None or mac_address == "":
                sn_hex = "".join("{:02x}".format(ord(c)) for c in self._lockData["lockSn"])
                if mqtt_advert[24:24+len(sn_hex)] != sn_hex:
                    return
                self._lockConfig[CONF_MAC_ADDRESS] = mqtt_mac
                self.requestDetails(mqtt_mac)

            if mqtt_mac == self._lockConfig[CONF_MAC_ADDRESS] and len(mqtt_advert) == 62:
        ...
        if "BLEOperation" in payload:
            if "state" and "MAC" in payload["BLEOperation"]:
                if payload["BLEOperation"]["state"] != "DONEWRITE":
                    _LOGGER.error("Failed sending frame: returned %s", payload["BLEOperation"]["state"])
                    self.curr_state = LOCK_STATE_FAILED
                    raise Exception("Failed sending frame: returned %s", payload["BLEOperation"]["state"])
                    return

                if (
                    payload["BLEOperation"]["MAC"] == self._lockConfig[CONF_MAC_ADDRESS]
    ...
    async def operateLock(self, lock_dir):
        ...
        if self.operating != 0:
            barr = self.getOperationCode(lock_dir)
            self.sendframe1()
    ...
    def requestDetails(self, mac_addr):
        mqtt.publish(self.hass, BLEDetailsTopic % self._lockConfig[CONF_MQTT_TOPIC], mac_addr)

    def scanAllAdverts(self):
        mqtt.publish(self.hass, BLEDetailsAllTopic % self._lockConfig[CONF_MQTT_TOPIC], "")
    ...
    def sendframe2(self):
        mqtt.publish(self.hass, BLEOpTopic % self._lockConfig[CONF_MQTT_TOPIC], self.BLEOPWritePAYLOADGen(self.frame2hex))

    def BLEOPWritePAYLOADGen(self, frame):
        mac_address = self._lockConfig[CONF_MAC_ADDRESS]
        payload = f"M:{mac_address} s:{serviceuuid} c:{writecharacteristicuuid} w:{frame} go"
        _LOGGER.debug("Sending payload [ %s ]", payload)
        return payload

    def BLEOPreadPAYLOADGen(self):
        return f"M:{self._lockData['mac_address']} s:{serviceuuid} c:{readcharacteristicuuid} r go"
    ...
    def parse_MQTT_advert(self, mqtt_advert):
        ...
        if serialnumber != self._lockConfig["sn"]:
            _LOGGER.error("ERROR: s/n in advert (%s) is different from cloud data (%s)", serialnumber, self._lockConfig["sn"])
    ...
    def getOperationCode(self, lock_dir):
